...
class Taxonomy_Core {
	...
	public function __construct( $taxonomy, $arg_overrides = array(), $object_types = array( 'post' ) ) {
		...
		if ( ! is_array( $taxonomy ) ) {
			wp_die( __( 'It is required to pass a single, plural and slug string to Taxonomy_Core', 'chad' ) );
		}
	...
	public function get_args() {
		...
		$labels = array(
			...
			'parent_item'       => isset( $this->arg_overrides['hierarchical'] ) && $this->arg_overrides['hierarchical'] ? sprintf( __( 'Parent %s', 'chad' ), $this->singular ) : null,
			...
			'edit_item'         => sprintf( __( 'Update %s', 'chad' ), $this->singular ),
			'add_new_item'      => sprintf( __( 'Add New %s', 'chad' ), $this->singular ),
			'add_new_item'      => sprintf( __( 'New %s Name', 'chad' ), $this->singular ),
		);

		$hierarchical = true;

		if ( isset( $args['hierarchical'] ) && $args['hierarchical'] == false ) {
			$labels['popular_items']              = sprintf( __( 'Popular %s', 'chad' ), $this->plural );
			$labels['separate_items_with_commas'] = sprintf( __( 'Separate %s with commas', 'chad' ), $this->plural );
			$labels['add_or_remove_items']        = sprintf( __( 'Add or remove %s', 'chad' ), $this->plural );
			$labels['choose_from_most_used']      = sprintf( __( 'Choose from the most used %s', 'chad' ), $this->plural );
			$hierarchical = false;
		}
