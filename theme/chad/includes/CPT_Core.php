...
if ( ! class_exists( 'CPT_Core' ) ) :
	...
	class CPT_Core {
		...
		public function __construct( array $cpt, $arg_overrides = array() ) {
			...
			if ( ! is_array( $cpt ) ) {
				wp_die( __( 'It is required to pass a single, plural and slug string to CPT_Core', 'chad' ) );
			}
		...
		public function get_args() {
			...
			$labels = array(
				...
				'add_new'            => sprintf( __( 'Add New %s', 'chad' ), $this->singular ),
				'add_new_item'       => sprintf( __( 'Add New %s', 'chad' ), $this->singular ),
				'edit_item'          => sprintf( __( 'Edit %s', 'chad' ), $this->singular ),
				'new_item'           => sprintf( __( 'New %s', 'chad' ), $this->singular ),
				'all_items'          => sprintf( __( 'All %s', 'chad' ), $this->plural ),
				'view_item'          => sprintf( __( 'View %s', 'chad' ), $this->singular ),
				'search_items'       => sprintf( __( 'Search %s', 'chad' ), $this->plural ),
				'not_found'          => sprintf( __( 'No %s', 'chad' ), $this->plural ),
				'not_found_in_trash' => sprintf( __( 'No %s found in Trash', 'chad' ), $this->plural ),
				'parent_item_colon'  => isset( $this->arg_overrides['hierarchical'] ) && $this->arg_overrides['hierarchical'] ? sprintf( __( 'Parent %s:', 'chad' ), $this->singular ) : null,
		...
		public function messages( $messages ) {
			...
			$cpt_messages = array(
				...
				2 => __( 'Custom field updated.', 'chad' ),
				3 => __( 'Custom field deleted.', 'chad' ),
				4 => sprintf( __( '%1$s updated.', 'chad' ), $this->singular ),
				/* translators: %s: date and time of the revision */
				5 => isset( $_GET['revision'] ) ? sprintf( __( '%1$s restored to revision from %2$s', 'chad' ), $this->singular , wp_post_revision_title( (int) $_GET['revision'], false ) ) : false,
				7 => sprintf( __( '%1$s saved.', 'chad' ), $this->singular ),
			);

			if ( $this->get_arg( 'public' ) ) {

				$cpt_messages[1] = sprintf( __( '%1$s updated. <a href="%2$s">View %1$s</a>', 'chad' ), $this->singular, esc_url( get_permalink( $post_ID ) ) );
				$cpt_messages[6] = sprintf( __( '%1$s published. <a href="%2$s">View %1$s</a>', 'chad' ), $this->singular, esc_url( get_permalink( $post_ID ) ) );
				$cpt_messages[8] = sprintf( __( '%1$s submitted. <a target="_blank" href="%2$s">Preview %1$s</a>', 'chad' ), $this->singular, esc_url( add_query_arg( 'preview', 'true', get_permalink( $post_ID ) ) ) );
				// translators: Publish box date format, see http://php.net/date
				$cpt_messages[9] = sprintf( __( '%1$s scheduled for: <strong>%2$s</strong>. <a target="_blank" href="%3$s">Preview %1$s</a>', 'chad' ), $this->singular, date_i18n( __( 'M j, Y @ G:i', 'chad' ), strtotime( $post->post_date ) ), esc_url( get_permalink( $post_ID ) ) );
				$cpt_messages[10] = sprintf( __( '%1$s draft updated. <a target="_blank" href="%2$s">Preview %1$s</a>', 'chad' ), $this->singular, esc_url( add_query_arg( 'preview', 'true', get_permalink( $post_ID ) ) ) );

			} else {

				$cpt_messages[1] = sprintf( __( '%1$s updated.', 'chad' ), $this->singular );
				$cpt_messages[6] = sprintf( __( '%1$s published.', 'chad' ), $this->singular );
				$cpt_messages[8] = sprintf( __( '%1$s submitted.', 'chad' ), $this->singular );
							// translators: Publish box date format, see http://php.net/date
				$cpt_messages[9] = sprintf( __( '%1$s scheduled for: <strong>%2$s</strong>.', 'chad' ), $this->singular, date_i18n( __( 'M j, Y @ G:i', 'chad' ), strtotime( $post->post_date ) ) );
				$cpt_messages[10] = sprintf( __( '%1$s draft updated.', 'chad' ), $this->singular );
		...
		public function title( $title ){
			...
			if ( isset( $screen->post_type ) && $screen->post_type == $this->post_type )
				return sprintf( __( '%s Title', 'chad' ), $this->singular );
